...
export class UpdateAppointmentComponentP implements OnInit {
  ...
  constructor(private apService:AppointmentsService,
    ...
    ngOnInit(): void {
      ...
      this.apService.getAppointmentById(this.ap_id).subscribe(data => {
    ...
    onSubmit()
  {
    this.apService.updateAppointment(this.ap_id,this.appointmentlist).subscribe( data =>{
      this.registerAppointment();      
    }, error =>console.log(error));
  }
  registerAppointment(){
  
    this.router.navigate(['/doctorassigned']);
